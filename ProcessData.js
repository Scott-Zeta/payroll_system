...
function parseShift (groupedShiftMap) {
  ...
  groupedShiftMap.forEach((value, key) => {
    ...
    value.forEach(record => {
      ...
      const {hoursRemain:weeklyRemain, overtimeResult:weeklyOTResult} = parseOvertime("Weekly", weeklyTotal, duration, CONFIG.OT_WEEKLY_TIME_THRESHOLD, CONFIG.OT_WEEKLY_THRESHOLD_WAGE);
      parseResult = {...parseResult, ...weeklyOTResult}
      // If there is time remain, parse for if there is daily overtime
      if (weeklyRemain > 0){
        const {hoursRemain: dailyRemain, overtimeResult:dailyOTResult} = parseOvertime("Daily", dailyTotal, weeklyRemain, CONFIG.OT_DAILY_TIME_THRESHOLD, CONFIG.OT_DAILY_THRESHOLD_WAGE);
        ...
        if (dailyRemain > 0){
          const regularWorkResult = parseRegularWork(record.date, record.start, dailyRemain, record.break);
...
function parseRegularWork(date, shiftStart, remainHours, breakHours){
  ...
  */
  ...
  const closeDecimal = timeToDecimal(CONFIG.CLOSE_TIME) === 0 ? 24 : timeToDecimal(CONFIG.CLOSE_TIME);
  ...
  regularWorkArray.forEach((element) => regularWorkResult[`${element.name}`] = {
    wage: element.wage,
    hours: element.hours,
...
function parseOvertime(prefix, totalHours, duration, thresholds, thresholds_wage) {
